...
class ICPNode(Node):
    def __init__(self):
        ...
        self.pcd_sub = self.create_subscription(PointCloud2, "/velodyne_points", self.pcd_callback, 10) # Task 3.3
        self.marker_pub = self.create_publisher(Marker, "/marker", 10) # Task 3.4
    ...
    def pcd_callback(self, msg: PointCloud2):
        ...
        ### Task 3.5 ###
        N = pts.shape[0]
        new_pts = np.hstack((pts, np.ones((N, 1))))
        transformed_pts = self.lidar_pose @ new_pts.T
        transformed_pts = transformed_pts.T
        transformed_pts = transformed_pts[:, :3]
        pt_cloud = o3d.geometry.PointCloud()
        pt_cloud.points = o3d.utility.Vector3dVector(transformed_pts)
        pt_cloud = pt_cloud.uniform_down_sample(10)
        ### TODO: Task 3.6 ###
        if self.use_open3d:
            self.transformation = open3d_icp(pt_cloud, self.prev_pcd, self.transformation)
        ### Task 3.6 ###
        else:
            source_np = np.asarray(pt_cloud.points)
            dest_np = np.asarray(self.prev_pcd.points)
            self.transformation = icp(source_np, dest_np, self.transformation)
        ### TODO: Task 3.7 ###
        ...
        ### TODO: Task 3.8 ###
        self.marker.pose.position.x = self.pose[0, 3] / self.pose[3, 3]
        self.marker.pose.position.y = self.pose[1, 3] / self.pose[3, 3]
        self.marker_pub.publish(self.marker)
        self.prev_pcd = pt_cloud
        ### Task 3.8 ###
