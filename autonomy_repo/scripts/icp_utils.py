...
def nearest_neighbor(src: np.ndarray, dst: np.ndarray, radius=0.01):
    ...
    n = NearestNeighbors(n_neighbors = 1, radius = radius, algorithm = 'auto').fit(dst)
    distances, indices = n.kneighbors(src, return_distance = True)
    ########### YOUR CODE ENDS HERE ###########
...
def icp(
    ...
    for i in trange(max_iterations):
        dist, ind = nearest_neighbor(src[:m, :], dst[:m, :].T, radius = knn_radius)
        T, _, _ = best_fit_transform(src[:m, :].T, dst[:m, ind].T)
        src = np.dot(T, src)
        cur_err = np.mean(dist)
        if abs(prev_error - cur_err) < tolerance:
            break
        prev_error = cur_err
    ########### YOUR CODE ENDS HERE ###########
